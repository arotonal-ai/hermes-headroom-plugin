...
The native Git launcher accepts the same subcommands. A successful doctor reports `RUNTIME_FULL_DURABLE` only when the pinned upstream status is semantically `running` and healthy for the expected profile/preset/port, the matching native supervisor contract is exact, readiness passes, and sentinel retrieval succeeds. Native Windows uses a manifest-owned no-console launcher and verifies both enabled Task Scheduler actions/triggers plus launcher hash. `reconcile --dry-run` is a strict zero-write inventory and never adopts a healthy listener without manager-state, manifest, runtime, upstream lifecycle and supervisor identity. Legacy environment-mutation history returns `REINSTALL_REQUIRED`; it is preserved for symmetric rollback and is never erased to force adoption. `--apply` remains limited to the manager-owned task-contract migration. Exit code `0` alone is not lifecycle evidence. If setup or remove is incomplete, manager state and private logs are preserved for rollback. Full contract and gated upgrade procedure: [docs/runtime-manager.md](docs/runtime-manager.md).
