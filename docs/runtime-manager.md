...
`reconcile --dry-run --json` is a strict zero-write inventory: it does not acquire the transaction lock, append runtime logs, download packages, rewrite state/manifest files, connect to the listener, or touch the supervisor. The `headroom-reconcile-plan-v1` payload reads manager state, purge marker, runtime executables and pinned state versions, the complete manifest mismatch set, native supervisor identity, and Windows OS socket/process tables independently. It deliberately does **not** call `/readyz` or `headroom install status`, because application-level discovery can append to proxy/access logs. The payload reports both probes as `not_probed_read_only`. A listener PID is ownership evidence only when its OS image is the exact managed venv executable, or when Windows reports the exact base interpreter declared by that managed venv's `pyvenv.cfg` and the original command image is the exact managed venv launcher. A command image never establishes ownership for an arbitrary process image. Process arguments are discarded before output. All independent manager identities must also match; unknown, inaccessible, multiple, non-loopback, or mismatched listener identity fails closed.
