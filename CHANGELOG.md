...
- Add a default-off, profile-isolated, content-addressed local exact fallback with TTL/quota, checksum verification and explicit exact/redacted/expired states; files use `0700`/`0600` on POSIX and the active user's inherited profile ACL on Windows.
