...
class IsolatedUnitRunnerTest(unittest.TestCase):
    def test_isolated_environment_drops_headroom_state_and_preserves_unrelated_env(self):
        ...
        try:
            ...
            env = MODULE.isolated_environment(Path("/isolated/test-home"))
        finally:
            os.environ.clear()
            os.environ.update(old)

        self.assertFalse(any(key.startswith("HEADROOM_") for key in env))
        self.assertEqual(env["HOME"], "/isolated/test-home")
        self.assertEqual(env["USERPROFILE"], "/isolated/test-home")
        self.assertEqual(env["HERMES_HOME"], "/isolated/test-home/.hermes")
