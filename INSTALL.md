...
Compatibility: v0.6.0rc1 preserves the published v0.5.2 managed default, `headroom-ai[proxy]==0.32.1` plus `litellm==1.91.3`, and adds P1–P3 context-lifecycle capability without changing provider routing. Headroom 0.31.0 remains a plugin-compatibility/rollback lane, not the managed install default. Use `--spec` / `HEADROOM_AI_SPEC` or `--litellm-spec` / `HEADROOM_LITELLM_SPEC` only for an explicit rollback, target-host diagnostic, or isolated canary. See [docs/compatibility.md](docs/compatibility.md); Python 3.13/3.14 and newer dependency ranges remain experimental.
